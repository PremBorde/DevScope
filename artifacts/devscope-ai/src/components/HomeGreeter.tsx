...
export default function HomeGreeter({ message = "Hello! Welcome 👋" }: Props) {
  ...
  useEffect(() => {
    ...
    const t4 = setTimeout(() => setPhase("done"),     6600);
    return () => {
  ...
  const armStyle = (dir: "l" | "r", origin: string): React.CSSProperties => {
    ...
    return { transformOrigin: origin, animation: `walk-arm-${dir} 0.38s ease-in-out infinite alternate` };
  };

  const legStyle = (dir: "l" | "r", origin: string): React.CSSProperties => {
    if (!isWalking) return { transformOrigin: origin };
    return { transformOrigin: origin, animation: `walk-leg-${dir} 0.38s ease-in-out infinite alternate` };
  };
    ...
    >
      ...
      >
        ...
        >
          {message}
          {/* Bubble tail — outer */}
          <div style={{ position: "absolute", bottom: -10, left: 18, width: 0, height: 0, borderLeft: "7px solid transparent", borderRight: "7px solid transparent", borderTop: "10px solid #000" }} />
          {/* Bubble tail — inner */}
          <div style={{ position: "absolute", bottom: -6, left: 20, width: 0, height: 0, borderLeft: "5px solid transparent", borderRight: "5px solid transparent", borderTop: "7px solid #fff" }} />
        </div>
      </div>
