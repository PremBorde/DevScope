...
if (isProd) {
  // Use relative path from the build artifact (dist/index.mjs)
  // In monorepo: backend/dist -> backend/../frontend/dist
  const frontendDistPath = path.resolve(__dirname, "../../frontend/dist/public");

  if (fs.existsSync(frontendDistPath)) {
    app.use(express.static(frontendDistPath));

    // SPA routing: serve index.html for all non-API requests
    ...
    logger.warn(
      ...
      "Frontend dist folder not found. Static serving disabled.",
    );
  }
}
